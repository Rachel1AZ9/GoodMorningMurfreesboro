...
- **Forms** — All forms (guest-spot, nonprofit-request, general-contact, and the newsletter in every page footer) POST to a Google Apps Script web app (`apps-script/Code.gs` is the source of truth) which verifies a Cloudflare Turnstile token and emails `hello@goodmorningmurfreesboro.com`. Each page that has the newsletter form also includes the Turnstile script tag in `<head>` and an inline IIFE right after the form that handles the submit.
